...
У нас есть массив, чтобы отрисовать всё карточки магазина, нужно перебрать массив, для каждого элемента создать карточку и вставить её контейнер

Используя метод массив, перебираем его, получает доступ к каждому его элементу. Потом Вызываем функцию, которую ты уже написала(передаем туда tittle элемента, его описание и остальные параметры,) Эта функция должна ВЕРНУТЬ заполненный темплейт, который нужно записать в переменную. И эту переменную уже вставить в контейнер